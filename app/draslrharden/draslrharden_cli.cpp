...
static droption_t<bool> fail_address_leaks(DROPTION_SCOPE_CLIENT, "fail_address_leaks", false,
                            ...
                            bool for_trace, bool translating, void *user_data)
{
    ...
    auto argc = drreg_reservation{drcontext, bb, instr};
    auto argv = drreg_reservation{drcontext, bb, instr};
    auto envp = drreg_reservation{drcontext, bb, instr};

#define MINSERT instrlist_meta_preinsert
    drmgr_disable_auto_predication(drcontext, bb);
    MINSERT(bb, instr, XINST_CREATE_load(drcontext,
                                         opnd_create_reg(argc),
                                         OPND_CREATE_MEM32(DR_REG_SP, 0)));
    MINSERT(bb, instr, INSTR_CREATE_add(drcontext,
                                        opnd_create_reg(argv),
                                        opnd_create_reg(DR_REG_SP),
                                        OPND_CREATE_INT(4)));
    MINSERT(bb, instr, INSTR_CREATE_add_shimm(drcontext,
                                              opnd_create_reg(envp),
                                              opnd_create_reg(argv),
                                              opnd_create_reg(argc),
                                              OPND_CREATE_INT(DR_SHIFT_LSL),
                                              OPND_CREATE_INT(2)));
    MINSERT(bb, instr, XINST_CREATE_add(drcontext,
                                        opnd_create_reg(envp),
                                        OPND_CREATE_INT(4)));
                         ...
                         bool for_trace, bool translating, void *user_data)
{
     ...
     */
    auto sreg1 = drreg_reservation{drcontext, bb, instr};
    auto sreg2 = drreg_reservation{drcontext, bb, instr};
    instrlist_meta_preinsert(bb, instr, XINST_CREATE_move(drcontext,
                                                          opnd_create_reg(sreg2),
                                                          OPND_CREATE_INT(TEXT_POINTER_TAINT)));
    drtaint_insert_reg_to_taint(drcontext, bb, instr, DR_REG_PC, sreg1);
    instrlist_meta_preinsert(bb, instr, XINST_CREATE_store_1byte(drcontext,
...
event_pre_syscall(void *drcontext, int sysnum)
{
    if (sysnum == SYS_write || sysnum == SYS_send)
    {
        ...
        {
            byte result;
            if (drtaint_get_app_taint(drcontext, (app_pc)&buffer[i], &result) && result != 0)
